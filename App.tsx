...
const SplashScreen: React.FC<{ onFinish: () => void }> = ({ onFinish }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] bg-xmas-dark flex flex-col items-center justify-center transition-opacity duration-500 ${fade ? 'opacity-0' : 'opacity-100'}`}>
       <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-xmas-red/20 to-xmas-dark pointer-events-none"></div>
       
       <div className="relative z-10 flex flex-col items-center">
         <div className="text-8xl mb-6 animate-bounce-slow filter drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]">🎁</div>
         <h1 className="text-6xl md:text-8xl font-christmas text-xmas-gold animate-fade-in drop-shadow-xl tracking-wider">
           Giftmacher
         </h1>
         <p className="text-white/70 mt-4 text-xl font-light tracking-widest animate-pulse">
           Happy New Year!
         </p>
       </div>
    </div>
...
const App: React.FC = () => {
    ...
    if (isPlaying) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(error => {
          console.log("Audio play failed (browser policy or load error):", error);
        });
      }
    }
  ...
  return (
    <div className="min-h-screen bg-xmas-dark flex flex-col items-center relative overflow-hidden overflow-y-auto">
      ...
      {/* Audio Element - Using reliable Wikimedia Commons Source */}
      <audio ref={audioRef} loop>
        <source src="https://upload.wikimedia.org/wikipedia/commons/8/8c/Jingle_Bells_Kevin_MacLeod_-_Christmas.mp3" type="audio/mpeg" />
      </audio>

      {/* Top Bar with Controls */}
      <div className="w-full max-w-6xl mx-auto flex justify-between sm:justify-end items-center p-4 z-50 relative gap-3">
          <button 
            onClick={toggleMusic}
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-all shadow-lg border border-slate-600 ${isPlaying ? 'bg-xmas-green text-white ring-2 ring-green-400/50' : 'bg-slate-800 text-slate-300'}`}
          >
            {isPlaying ? (
              <>
